...
namespace OpticsSerializer
{
    public class SpectralSerializer
    {
        ...
        public class Optical
        {
            ...
            public double CalculateTransmissionIntegral(Optical reference)
            {
                double sum = 0, refsum = 0;
                foreach (SpectralSerializer.Datapoint d in reference.dataPoints)
                {
                    double y = FindDataPointByWaveLength(d.incidence.wavelengths.wavelength);
                    if (y == -1)
                    {
                        return -1;
                    }
                    ...
                    {
                        sum += d.results.transmittance;
                        refsum += d.results.transmittance * y;
                    }
                }
                return refsum / sum;
            }

            private double FindDataPointByWaveLength(double wavelegth)
            {
                foreach (SpectralSerializer.Datapoint d in dataPoints)
                {
                    if (d.incidence.wavelengths.wavelength == wavelegth)
                    {
                        return d.results.transmittance;
                    }
                }
                return -1;
            }
        ...
        public class Datapoint
        {
            public Datapoint(double wavelength, double value, Optical.MeasurementType type, Optical.ScatterDetectionDefinition direction)
            {
                ...
                if (type == Optical.MeasurementType.transmittance)
                {
                    this.results.transmittance = value;
                }
                else if (type == Optical.MeasurementType.reflectance)
                {
                    this.results.reflectance = value;
                }
            }
